...
| [summerlin-campus-embodied-agent.md](summerlin-campus-embodied-agent.md) | Downtown Summerlin campus | **One embodied agent for the campus** (One Summerlin, Two Summerlin, Meridian) — decided 09/06 as the replacement for the 1-embodied-plus-4-experts pattern. Carries the same-day expertise of the plant agents (gates, data quality, run state, within-unit circuit imbalance, loop divergence) and leaves 30-day trends to the Bldg J PdM. 06:00 PT, reports on yesterday, one line per building when all OK, EMAIL on deviation. **v0.6, deploy candidate after five Sonnet rehearsals (09/06 x3, 09/18 x2); the v0.4 false SEVERE on flat space points is fixed and regression-passed.** Records: `summerlin-campus-rehearsal-2026-09-06.md`, `-2026-09-18.md` | Building + PO UUIDs, ~60 sensor UUIDs, device instances, PEG/connector names |
| `summerlin-campus-embodied-bindings.csv` | The 229-row binding table behind it — Tier A daily sentinels, Tier B Bldg J rotation, profile setpoints, canaries | Sensor UUIDs, device instances, sample values |
